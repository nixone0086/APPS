...
class Char8x14 : public GraphElement {
public:
    Point2D m_pos;
    char m_char;
    Char8x14(Point2D t_pos, char t_char, RGB t_fg, RGB t_bg)
        : GraphElement(t_fg, t_bg), m_pos(t_pos), m_char(t_char) {}

    void draw() override {
        const uint32_t* ch_bitmap = font[(uint8_t)m_char]; // Assuming font is defined elsewhere
        for (int row = 0; row < 14; ++row) {  // 14 rows instead of 36
            uint32_t row_data = ch_bitmap[row];
            for (int col = 0; col < 8; ++col) {  // 8 columns instead of 22
                if (row_data & (1 << col)) {
                    uint16_t rgb565 = ((m_fg_color.r >> 3) << 11) |
                                      ((m_fg_color.g >> 2) << 5) |
                                      (m_fg_color.b >> 3);
